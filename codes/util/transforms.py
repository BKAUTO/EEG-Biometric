...
class gammaFilter(object):
    ...
    def __call__(self, data1):
        ...
        for channel in range(data.shape[0]):
            ...
            out[channel,:] = self.firwin_bandpass_filter(out[channel,:], self.ntaps, self.band[0], self.band[1], self.fs)
...
class Energy_Wavelet(object):
    def __init__(self, lowcut=0.16, highcut=78, bands=[[0.17,4],[4,8],[8,14],[14,32],[32,64],[64,78]], fs=160, ntaps=128):
        self.lowcut = lowcut
        self.highcut = highcut
        self.bands = bands
        self.fs = fs
    ...
    def __call__(self, data1):
        ...
        for channel in range(data.shape[0]):
            ...
            for i in range(len(self.bands)):
                ...
                energys.append(sum(x**2 for x in self.firwin_bandpass_filter(temp, self.ntaps, self.bands[i][0], self.bands[i][1], self.fs)))
